...
class RGBDDeepPixBiSGAPMHCrossModal(nn.Module):
    ...
    def __init__(self, pretrained=True, num_channels=4):
        ...
        num_channels: int
            Number of channels in the input. 
        """
        super(RGBDDeepPixBiSGAPMHCrossModal, self).__init__()
    ...
    def forward(self, img):
        ...
        gap_rgb = self.gavg_pool(enc_rgb).squeeze() # nn.AvgPool2d(kernel_size = 384, stride = 0, padding = 0, ceil_mode=False, count_include_pad=True)(enc).squeeze()

        gap_d = self.gavg_pool(enc_d).squeeze() # nn.AvgPool2d(kernel_size = 384, stride = 0, padding = 0, ceil_mode=False, count_include_pad=True)(enc).squeeze()

        gap_d=gap_d.view(-1,384)

        gap_rgb=gap_rgb.view(-1,384)
        
        gap_rgb = nn.Sigmoid()(gap_rgb) # or nn.ReLU()

        gap_d = nn.Sigmoid()(gap_d) # or nn.ReLU()
...
# model=RGBDDeepPixBiSGAPMHCrossModal(pretrained=True,num_channels=4)
